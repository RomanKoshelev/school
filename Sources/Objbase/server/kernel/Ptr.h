...
// включительно (from<=to). По умолчанию to равен from.
   public: void Del (int from, int to=-1)
	{
		DEBUG_STACK_NAME (Del);
	
		if (to==-1) to=from;

		SERVER_DEBUG_ASSERT (m_nSize!=0,			"CPtr::Del(from,to) -- Массив пуст");
		SERVER_DEBUG_ASSERT (from<=to,			"CPtr::Del(from,to) -- Кривой диапазон");
		SERVER_DEBUG_ASSERT (from>=0,				"CPtr::Del(from,to) -- Кривой диапазон");
		SERVER_DEBUG_ASSERT (to<(int)m_nSize,	"CPtr::Del(from,to) -- Кривой диапазон");

		int i,j;
		for (i=from, j=to+1; j<(int)m_nSize; i++,j++)
		{
			m_pData[i] = m_pData[j];
		}

		Realloc (m_nSize - (to-from+1));
	}
...
// CPtr:: Вставить элемент в нужное место в отсортированном массиве
// Возвращает позицию, в которую вставлен элемент
	public:	int AddSorted (const T& value)
	{
   ...
   public: void MoveToStart (int from, int to=-1)
	{
		...
		if (from == 0) return;
		if (to==-1) to=(int)m_nSize-1;
		SERVER_DEBUG_ASSERT (from<=to,	"MoveToStart: to<from");
		SERVER_DEBUG_ASSERT (from>=0,		"MoveToStart: from<0");
...
// Заполнение освободившегося куска DefValue.
   public: void MoveToStart  (int from, int to, const T& DefValue)
	{
		DEBUG_STACK_NAME (MoveToStart);
		SERVER_DEBUG_ASSERT (to<(int)m_nSize,	"MoveToStart: to>=m_nSize");
		SERVER_DEBUG_ASSERT (from<=to,			"MoveToStart: to<from");
		SERVER_DEBUG_ASSERT (from>=0,				"MoveToStart: from<0");
		MoveToStart (from, to);

		for (int i = to-from+1; i<(int)m_nSize; i++)
			m_pData[i] = DefValue;
	}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - [] 
   ...
   public: void MoveToEnd (int from, int to)
	{
		...
		if (to==(int)(m_nSize-1)) return;
		SERVER_DEBUG_ASSERT (to<(int)m_nSize,	"MoveToEnd: to>=m_nSize");
		SERVER_DEBUG_ASSERT (from<=to,			"MoveToEnd: to<from");
		SERVER_DEBUG_ASSERT (from>=0,				"MoveToEnd: from<0");

		int i,j;
		for (i=(int)m_nSize-1, j=to; j>=from; i--, j--)
		{
			m_pData[i] = m_pData[j];
		}
	}
...
// Переместить кусок в конец массива без изменения его размера.
// Заполнение освободившегося куска DefValue.
   public: void MoveToEnd (int  from, int to, const T& DefValue)
	{
		DEBUG_STACK_NAME (MoveToEnd);
		SERVER_DEBUG_ASSERT (to<(int)m_nSize,	"MoveToEnd: to>=m_nSize");
		SERVER_DEBUG_ASSERT (from<=to,			"MoveToEnd: to<from");
		...
		for (int i = 0; i < (int)m_nSize-(to-from+1); i++)
			m_pData[i] = DefValue;
	}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - [] 
...
//______________________________________________________________________________
//                                                                            []
//` Вызов в цикле заданного метода для каждого элемента массива               []                                                           
//                                                                            []
#define FOR_EACH_ITEM(_arr,_method)							{for (int i=0; i<_arr.Size(); i++){_arr[i]._method();					}}
#define FOR_EACH_ITEM_ARG_1(_arr,_method,a1)				{for (int i=0; i<_arr.Size(); i++){_arr[i]._method(a1);				}}
#define FOR_EACH_ITEM_ARG_2(_arr,_method,a1,a2)			{for (int i=0; i<_arr.Size(); i++){_arr[i]._method(a1,a2);			}}
#define FOR_EACH_ITEM_ARG_3(_arr,_method,a1,a2,a3)		{for (int i=0; i<_arr.Size(); i++){_arr[i]._method(a1,a2,a3);		}}
#define FOR_EACH_ITEM_ARG_4(_arr,_method,a1,a2,a3,a4)	{for (int i=0; i<_arr.Size(); i++){_arr[i]._method(a1,a2,a3,a4);	}}
#define FOR_EACH_ITEM_EVAL(_arr,_code)						{for (int i=0; i<_arr.Size(); i++){_code}}
//____________________________________________________________________________[]
