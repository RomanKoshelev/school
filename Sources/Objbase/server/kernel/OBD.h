...
// OBD_Exist
bool OBD_Exist (long classID, identifier objID, identifier randID=0);
...
// OBD_DeleteObject
void OBD_DeleteObject (long classID, identifier objID, bool bIgnoreWarning=false);
...
// идентификатором и уникальным номером.
bool OC_Exist (identifier classID, identifier objID, identifier randID=0);
...
{
	DEBUG_STACK_NAME (OBD_GetObjectFieldValue);
...
{
	DEBUG_STACK_NAME (OBD_GetObjectFieldArrValue);
...
{
	DEBUG_STACK_NAME (OBD_UpdateIndex);
...
// Не удалось найти объект в индексе 
	if (pData_m_CoIndexPtr(classID, pData, indexID)==NULL)
	{
		SERVER_DEBUG_ERROR_3 ("OBD_UpdateIndex: Не удалось найти объект %d класса %s в индексе %d", objID, className[classID], indexID);
		return;
	}
...
// Пробегаемся по всем ключам, составляющим индекс indexID
	...
	// Если ключ -- изменившееся поле, его значение надо заменить в индексе
		...
		// Удаляем старый ключ
			...
			if (bRes==false)
			{
				SERVER_DEBUG_ERROR_3 ("OBD_UpdateIndex: Не удалось удалить ключ объекта %d класса %s из дерева индексов %d", objID, className[classID], indexID);
			}
		...
		// Не удалось обновить индекс
			if (pData_m_CoIndexPtr(classID, pData, indexID)==NULL)
			{
				SERVER_DEBUG_ERROR_2 ("OBD_UpdateIndex: Не удалось обновить индекс %d в объекте класса '%s'", indexID, className[classID]);
				return;
			}
		}
	}
...
{
	DEBUG_STACK_NAME (OBD_PutObjectFieldValue);
...
{
	DEBUG_STACK_NAME (OBD_PutObjectFieldArrValue);
...
{
	DEBUG_STACK_NAME (OBD_GetObjectFieldArrSize);
...
{
	DEBUG_STACK_NAME (OBD_AddObjectFieldArrItem);
...
{
	DEBUG_STACK_NAME (OBD_AddFromObjectFieldArrItem);
...
{
	DEBUG_STACK_NAME (OBD_InsertObjectFieldArrItem);
...
{
	DEBUG_STACK_NAME (OBD_InsertSortedObjectFieldArrItem);
...
{
	DEBUG_STACK_NAME (OBD_FindObjectFieldArrItem);
...
//                                                                            []
template <class OD, class FT>
int OBD_DelObjectFieldArrItem (long classID, OBD_pointer coPtr, CPtr<FT> OD::* fieldArr, int from, int to)
{
	DEBUG_STACK_NAME (OBD_DelObjectFieldArrItem);
...
//______________________________________________________________________________
//                                                                            []
//` Move items to start or end                                                []                          
//                                                                            []
//----------------------------------------------------------------------------[] 
#define MOVE_TO_START	0
#define MOVE_TO_END		1
//----------------------------------------------------------------------------[] 
template <class OD, class FT>
void OBD_ObjectFieldArr_MoveToStartEnd (long classID, OBD_pointer coPtr, CPtr<FT> OD::* fieldArr, int from, int to, int Type)
{
	DEBUG_STACK_NAME (OBD_ObjectFieldArr_MoveToStartEnd);
...
// Предотвращаем изменение объекта
	...
	if (Type==MOVE_TO_START)
		(pData->*fieldArr).MoveToStart (from, to);
	else
		(pData->*fieldArr).MoveToEnd (from, to);
...
//----------------------------------------------------------------------------[] 
template <class OD, class FT>
void OBD_ObjectFieldArr_MoveToStartEnd (long classID, OBD_pointer coPtr, CPtr<FT> OD::* fieldArr, int from, int to, FT value, int Type)
{
	DEBUG_STACK_NAME (OBD_ObjectFieldArr_MoveToStartEnd);
	IF_OBJECT_INVALID {return;}
...
// Предотвращаем изменение объекта
	...
	if (Type==MOVE_TO_START)
		(pData->*fieldArr).MoveToStart (from, to, value);
	else
		(pData->*fieldArr).MoveToEnd (from, to, value);
...
{
	DEBUG_STACK_NAME (OBD_DelObjectFieldArrAllItems);
...
{
	DEBUG_STACK_NAME (OBD_CopyObjectFieldArrAllItemsFromPtr);
...
{
	DEBUG_STACK_NAME (OBD_CopyObjectFieldArrAllItemsToCPtr);
...
//______________________________________________________________________________
//                                                                            []
// Get reference to internal CPtr															[]
//                                                                            []
template <class OD, class FT>
const CPtr<FT>& OBD_GetObjectFieldArrDataReference (long classID, OBD_pointer coPtr, CPtr<FT> OD::* fieldArr)
{
	DEBUG_STACK_NAME (OBD_GetObjectFieldArrDataReference);

	IF_OBJECT_INVALID {static const CPtr<FT> defval; return defval;}
	OD* pData = (OD*)aObject->pData;
	return pData->*fieldArr;
}
//____________________________________________________________________________[]
...
{
	DEBUG_STACK_NAME (OBD_GetObjectFieldArrDataPointer);
