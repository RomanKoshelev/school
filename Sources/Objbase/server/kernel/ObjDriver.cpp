...
void CObjDriver::PrintFormat ()
{
	...
	for (CPtr<CObjFormat>::Iterator objtype=m_FormatArr.begin(); objtype!=m_FormatArr.end(); objtype++)
	{
		...
		printf (objtype->isVirtual? "virtual ": "");
		printf ("class ");
...
void CObjDriver::SaveFormat ()
{
	...
	for (CPtr<CObjFormat>::Iterator objtype=m_FormatArr.begin(); objtype!=m_FormatArr.end(); objtype++)
	{
		...
		formatFile.WriteData (objtype->isVirtual);
...
void CObjDriver::LoadFormat ()
{
	...
	for (i=0; i<formatNumber; i++)
	{
		...
		formatFile.ReadData		(format.isVirtual);
...
{
	DEBUG_STACK_NAME (SaveIndexes);

	if (PRINT_PROGRESS_IND) puts ("\n\n\n\n\n\n");
	for (CPtr<CObjFormat>::Iterator objtype=m_FormatArr.begin(); objtype!=m_FormatArr.end(); objtype++)
	{
		if (PRINT_PROGRESS_IND && objtype->indexArr.GetSize())
		{
			printf ("%s::SaveIndexes(): ", objtype->name.c_str());
		}
		size_t nPrevItems = NA;
		identifier indexID=0; 
		for (CPtr<CObjIndex>::Iterator index=objtype->indexArr.begin(); index!=objtype->indexArr.end(); index++) 
		{
			if (PRINT_PROGRESS_IND) 
			{
				printf ("\n    #%3d [MM: %d Mb]", indexID+1, MEMORY_MANAGER_GET_SIZE()/1024/1024);
			}
			if (index->bChanged)
			{
...
{
	DEBUG_STACK_NAME (LoadIndexes);

	for (CPtr<CObjFormat>::Iterator objtype=m_FormatArr.begin(); objtype!=m_FormatArr.end(); objtype++)
	{
		if (PRINT_PROGRESS_IND && objtype->indexArr.GetSize())
		{
			printf ("%s::LoadIndexes(): ", objtype->name.c_str());
		}
		size_t nPrevItems = NA;
		identifier indexID=0; 
		for (CPtr<CObjIndex>::Iterator index=objtype->indexArr.begin(); index!=objtype->indexArr.end(); index++)
...
{
	DEBUG_STACK_NAME (WriteIndexHelper);
...
{
	DEBUG_STACK_NAME (ReadIndexHelper);
...
size_t CObjIndex::Load ()
{
	...
	static CObjIndex_AVTree_Item Item;
	Item.Init ();
...
{
	TT_master,				//	master
	TT_virtual,				//	virtual -- не надо генерировать контроллеры
	TT_class,				//	class
	...
	for (it = Terms.begin(); it!=Terms.end() && CS!=CS_END; it++)
	{
			...
			case TT_Name: switch (CS)
				...
				// Имя класса
					case CS_CLASS_NAME_WAITING:
						...
						field.isArray	= false;
						field.name		= "randID";
						field.defValue	= "(identifier(rand()*time(0))/10+1)";
						format.fieldArr.Add (field);
						field.Erase();
		...
		//	master
			case TT_master: 
				if (format.isVirtual)
				{
					COMPILE_ERROR  ("Мастер-класс не может быть виртуальным.");
				}
				if (bExistMaster)
				{
							...
							for (j =0; j<(int)format._parentArr.GetSize(); j++)
							{
								...
								for (k=0; k<(int)m_FormatArr[parentID].fieldArr.GetSize(); k++) // по полям предка
								{
									...
									else
									{
										...
										if (strcmp(fieldName.c_str(),"classID") && strcmp(fieldName.c_str(),"randID")) if (strcmp(defValue1.c_str(), defValue2.c_str()) || (isArray1 != isArray2) || (nftype1 != nftype2))
										{
											static char SSS [1024];
		...
		//	Типы полей
			...
			case TT_guid:
				if (field.type == FT_nothing) field.type = FT_guid;
			case TT_int:
