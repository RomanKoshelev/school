...
//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * Comment end *
	...
	if (ReturnCode != BX_Query_Okay)								// Получить из списка строку запроса BX_Query.
	{
	...
	ReleaseArgList(ArgumentList);									// Освободить память из списка.
...
// Прием ответа от БД:												*
	...
	BX_Query[TotalLength] = 0;																			// Пометим конец запроса.
...
ReleaseArgList	(PBX_Arg		pArg)
{
	...
	while (pArg != NULL)
	{
		pArgNext	= pArg->pNextArg;				// сохраним указатель на следующую структуру

		free(pArg->ArgName);					// освободим имя аргумента

		if (pArg->ArgType == AT_String) {
			free(pArg->ArgVal.Arg_String);	// освободим строку-аргумент (если тип - строка)
		}
		
		free(pArg);								// освободим саму структуру

		pArg = pArgNext;							// перейдем к следующей
	}
	return;
}
//____________________________________________________________________________[]
