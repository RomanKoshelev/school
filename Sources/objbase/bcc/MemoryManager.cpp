...
#define MM_ERROR(str)											{char SSS[256]; sprintf (SSS, str							); puts(""); puts (SSS); puts(""); DBG_STOP;}
#define MM_ERROR_1(str, a1						)				{char SSS[256]; sprintf (SSS, str, a1						); puts(""); puts (SSS); puts(""); DBG_STOP;}
#define MM_ERROR_2(str, a1, a2				)				{char SSS[256]; sprintf (SSS, str, a1, a2					); puts(""); puts (SSS); puts(""); DBG_STOP;}
#define MM_ERROR_3(str, a1, a2, a3			)				{char SSS[256]; sprintf (SSS, str, a1, a2, a3			); puts(""); puts (SSS); puts(""); DBG_STOP;}
#define MM_ERROR_4(str, a1, a2, a3, a4		)				{char SSS[256]; sprintf (SSS, str, a1, a2, a3, a4		); puts(""); puts (SSS); puts(""); DBG_STOP;}
#define MM_ERROR_5(str, a1, a2, a3, a4, a5)				{char SSS[256]; sprintf (SSS, str, a1, a2, a3, a4, a5	); puts(""); puts (SSS); puts(""); DBG_STOP;}
//----------------------------------------------------------------------------[] 
...
#ifdef USE_RVK_MEMORY_MANAGER
//----------------------------------------------------------------------------[] 
inline void SET_FAT_ITEM (pbyte pFAT, int iBlock, byte bValue)
{
...
//----------------------------------------------------------------------------[] 
inline bool GET_FAT_ITEM (pbyte pFAT, int iBlock)
{
#ifndef USE_ONE_BIT_FOR_FAT
...
//----------------------------------------------------------------------------[] 
inline int GET_FIRST_FREE_FAT_ITEM (pbyte pFAT)
{
	int i,j;
#ifndef USE_ONE_BIT_FOR_FAT
...
class PageManager
{
	...
	public: ~PageManager ()
	{
#if 1  //{ RomanVK !!! -- нельзя освобождать страницы, на которых живут локальные переменные. // пробуем, однако, сделать это...
...
//----------------------------------------------------------------------------[] 
	...
	// Если есть возмжность найти не полностью занятую страничку
...
//----------------------------------------------------------------------------[] 
...
#endif
	{
		...
		if (iPage==-1)
		{
			MM_ERROR_2 ("PageManager[%d:%d] -- Can't alloc memory.", m_Number, BLOCK_SIZE[m_Number]);
			puts (MEMORY_MANAGER_REPORT(true));
			return NULL;
		}
...
protected: 
	...
	// Разместить не на странице (большие данные)
		if (nPageManager==OUTSIDE_PAGE_NUM)
		{
			...
			if (pData==NULL)
			{
				MM_ERROR_1 ("Can't alloc memory for out-page block. Size = %d", nSize+PARAGRAF_SIZE);
				puts (MEMORY_MANAGER_REPORT(true));
				return NULL;
			}
...
const char* MEMORY_MANAGER_REPORT(bool bIgnoreTime)
{
	int AVOID_PRINT_REPORT_AFTER_DAYS = 10;
	time_t MAX_TIME = 943363962/* 23 ноября 16:34 */ + 24*60*60 * AVOID_PRINT_REPORT_AFTER_DAYS;
	if (!bIgnoreTime && (time(0)>MAX_TIME))
		return ""; 
	if (pMemoryManager==NULL)
		return "\nMemory manager is off.\n"; 
	return pMemoryManager->GetReport();
}
...
const char* MEMORY_MANAGER_REPORT	(bool)	{return "\nMemory manager not supported.\n";}
