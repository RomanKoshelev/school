...
char * FT_NAME_SHORT	[]	= {"identifier",		"int",					"byte",					"bool",					"float",					"string"};
char * FT_NAME			[]	= {"identifier ",		"int        ",			"byte       ",			"bool       ",			"float      ",			"string     "};
char * FT_NAME_ARR	[]	= {"identifier_arr ", "int_arr        ",	"byte_arr       ",	"bool_arr       ",	"float_arr      ",	"string_arr     "};
char * FT_VOID				=  "void       ";
...
//	Классы
	...
	//	Очистить контроллер
		header += "void OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::Release (OBD_pointer _coPtr=NULL)");										header += SSS; NEW_LINE;
		sprintf (SSS, "{");																										header += SSS; NEW_LINE;
		sprintf (SSS, "   if(_coPtr!=NULL)");																				header += SSS; NEW_LINE;
		sprintf (SSS, "      OBD_ReleaseObject<OD_%s>(classID, _coPtr);",objtype->name.c_str());			header += SSS; NEW_LINE;
		sprintf (SSS, "   else if(coPtr!=NULL) {");																		header += SSS; NEW_LINE;
		sprintf (SSS, "      OBD_ReleaseObject<OD_%s> (classID, coPtr);",objtype->name.c_str());			header += SSS; NEW_LINE;
		sprintf (SSS, "     coPtr = NULL;");																				header += SSS; NEW_LINE;
		sprintf (SSS, "   }");																									header += SSS; NEW_LINE;
		sprintf (SSS, "}");																										header += SSS; NEW_LINE;
		NEW_LINE;
		NEW_LINE;
	...
	// Получить идентификатор первого объекта по текущему индексу
		...
		header += objtype->name.c_str();
		sprintf (SSS, "::First() const {return OBD_FirstIDInIndex<OD_%s> (classID, curIndexInfo);}",objtype->name.c_str());	header += SSS; NEW_LINE
	// Получить идентификатор последнего объекта по текущему индексу
		header += "identifier OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::Last() const {return OBD_LastIDInIndex<OD_%s> (classID, curIndexInfo);}",objtype->name.c_str());		header += SSS; NEW_LINE

	//	Отладочная печать
		NEW_LINE;
		header += "void OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::DebugPrint(string& dbg) const {OBD_DebugPrint<OD_%s>(classID, dbg, coPtr);}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	//	Отладочная печать
		...
		header += "void OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::DebugPrint() const {string dbg; OBD_DebugPrint<OD_%s>(classID, dbg, coPtr); dbg.Print();}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	// Проверка
		...
		header += "bool OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::IsValid () const {return OBD_IsObjectValid<OD_%s> (classID, coPtr);}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	// Проверка
		NEW_LINE;
		header += "OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::operator bool () const {return IsValid ();}");
		header += SSS;
		NEW_LINE;
		...
		header += "bool OC_";
		header += objtype->name.c_str();
		header += "::operator==(key_arr& ka)  {return Compare (ka)==0;}";			NEW_LINE;
		header += "bool OC_";
		header += objtype->name.c_str();
		header += "::operator< (key_arr& ka)  {return Compare (ka)< 0;}";			NEW_LINE;
		header += "bool OC_";
		...
		header += "int  OC_";
		header += objtype->name.c_str();
		sprintf(SSS, "::Compare   (key_arr& ka)  {return coPtr? OBD_CompareToKeyVarArr<OD_%s>(classID,curIndexInfo,coPtr,ka): -1;}",objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	// Сравнение == 
		...
		header += " ::operator== (const OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "& to) {return coPtr==to.coPtr;}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	// Сравнение идентификаторов
		header += "bool OC_";
		header += objtype->name.c_str();
		header += "::operator== (identifier objID)  {return objID == (coPtr? GetObjectID(): UNKNOWN);}";			NEW_LINE;
		header += "bool OC_";
		header += objtype->name.c_str();
		header += "::bool operator!= (identifier objID)  {return objID != (coPtr? GetObjectID(): UNKNOWN);}";	NEW_LINE;
		header += "OC_";
		header += objtype->name.c_str();
		header += "::operator identifier () const        {return coPtr? GetObjectID(): UNKNOWN;}";					 NEW_LINE;

	// Загрузка по идентификатору
		header += "void OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::Open (identifier objID) {if (coPtr!=NULL && (objID==GetObjectID())) return; Release(); if (objID==NA) return; coPtr=OBD_GetObject<OD_%s>(classID,objID);}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	// Принудительное сохранение объекта на диск
		header += "void OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::Save () {if (coPtr) OBD_SaveObject<OD_%s>(classID,coPtr);}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	// Принудительное сохранение текущего индекса на диск
		header += "void OC_";
		header += objtype->name.c_str();
   	header += "::SaveCurIndex () {OBD_SaveIndexToFile(classID,curIndexInfo);}"; NEW_LINE;
	// Принудительное сохранение всей базы на диск
		header += "void OC_";
		header += objtype->name.c_str();
   	header += "::SaveAllBase ()  {OBD_SaveAllData ();}"; NEW_LINE;

	//	Создание нового объекта
		header += "identifier OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::New () {Release(); coPtr=OBD_CreateObject<OD_%s>(classID); return GetObjectID();}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	//	Удаление объекта из базы
		header += "void OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::Delete () {OBD_DeleteObject<OD_%s>(classID,coPtr); coPtr=NULL;}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	//	Взять ID объекта
		header += "identifier OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::GetObjectID () const {return OBD_GetObjectID<OD_%s>(classID, coPtr);}", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
	...
	//	Взять ID типа
		header += "identifier OC_";
		header += objtype->name.c_str();
		sprintf (SSS, "::GetClassID () const {return OBD_GetClassID<OD_%s>(classID, coPtr);} ", objtype->name.c_str());
		header += SSS;
		NEW_LINE;
		NEW_LINE;
		NEW_LINE;
...
//	Включаем файл с драйвером базы объектов
	...
	header += "#include \"./../bcc/main.h\"";		NEW_LINE; // !!! -- наверное эти include уже не нужны
	header += "#include \"./../bcc/OBD.h\"";		NEW_LINE;
#ifdef USE_CObject
	header += "#include \"./../bcc/Object.h\"";	NEW_LINE;
#endif
...
//	Классы
	...
	// Простые поля
		bool bNL = true;
		for (f=0, field=objtype->fieldArr.begin(); field!=objtype->fieldArr.end(); field++, f++) if (strcmp (field->name.c_str(), "classID")) if (!field->isArray)
		{
			header += "// Поле:   ";
...
void CObjDriver::Create_ObjData_h (string& header)
{
	...
	header += "		memcpy (d_##field.GetPtr(), 															\\";		NEW_LINE;
	header += "			file.GetPtr()+nOffset, 																\\";		NEW_LINE;
...
//	Идентификаторы классов
	...
	for (i=0, objtype=m_FormatArr.begin(); objtype!=m_FormatArr.end(); objtype++, i++)
	{
		NEW_LINE;
...
void CreateLoadFromDisk (string& header, CPtr<CObjFormat>::Iterator& objtype)
{
	...
	// Проверяем идентификатор класса
		if (f==0) 
		{
			...
			header += "      {";																															NEW_LINE
			header += "         SERVER_DEBUG_ERROR_3 (\"Несовпадение типа при считывании объекта %d (тип!='%s'); tmp_classID==%d\", objID, className[classID], classID);";	NEW_LINE
...
void CreateDebugPrint (string& header, CPtr<CObjFormat>::Iterator& objtype)
{
	...
	header += "      dbg += \"\\n\\n------------------------------------------\\n\";"; NEW_LINE;
	sprintf (SSS, "      SSS.Format (\"[%%s:%%d]\\n\\n\", className[OBJ_CLASS_ID_%s], objID);", objtype->name.c_str());
	header += SSS;
	NEW_LINE;
	header += "      dbg += SSS;";
	NEW_LINE;
...
void CObjDriver::Create_ObjData_cpp (string& header)
{
	...
	header += "#define USES_ObjDriver_h";		NEW_LINE;
	NEW_LINE;
	header += "#include \"../bcc/main.h\"";	NEW_LINE;
	header += "#include \"../bcc/OBD.h\"";		NEW_LINE;
	header += "#include \"Lay_ObjData.h\"";	NEW_LINE;
	NEW_LINE;
	NEW_LINE;
...
//	Идентификаторы класов 
	NEW_LINE;
	NEW_LINE;
	header += "char * className[]=";	NEW_LINE;
	header += "{";	NEW_LINE;
...
void CObjDriver::Create_Interface_Dispatch_cpp	(string& header, const CPtr <INTERFACE_FORMAT>& Format, const string& Prefix)
{
	...
	header += "#define	USES_"; header += Prefix; header += "_Interface";	NEW_LINE;
	header += "#include	\""; header += Prefix; header += ".h\"";				NEW_LINE;
