...
#define CHECK_MAX_CACHED_ITEMS(MAX_CACHED_ITEMS)										\
	if (_cache_count>=(MAX_CACHED_ITEMS))													\
	{																									\
		key_arr ka;																					\
		ka += _fncrc; 																				\
		ka	+=	_fln;																					\
		CPtr<void*> _ptr4remove;																\
		for (_ptr=theArgResCache.Find(ka); 													\
			_ptr!=NULL; 																			\
			_ptr=theArgResCache.Next(_ptr))													\
		{																								\
			_pci=theArgResCache.ItemPointer(_ptr);											\
			if (CompareFunction_ARG_KEY (ka, _pci->m_arg)==0)							\
			{																							\
				_ptr4remove += _ptr;																\
			}																							\
		}																								\
		int _i;																						\
		for (_i=0;_i<_ptr4remove.GetSize();_i++)											\
		{																								\
			theArgResCache.Remove(_ptr4remove[_i]);										\
		}																								\
		_cache_count = 0;																			\
	}
//----------------------------------------------------------------------------[] 

#define USE_CACHE																			CACHE_ITEM _ci; void *_ptr=NULL; CACHE_ITEM *_pci=NULL; unsigned long _fncrc=string(__FILE__).GetCRC(); _ci+=_fncrc; int _fln=__LINE__; _ci+=_fln;	static int _cache_count=0;

#define IFCACHEDRETURN_BEG																_ptr=theArgResCache.Find(_ci.m_arg);	if(_ptr!=NULL){_pci=theArgResCache.ItemPointer(_ptr);_pci->BeginExtractArgs();
...
#define CACHE_STORE_RESULT_1(r1												)		}{_ci<<(r1);																											GOTO_IZVRAT_1 (r1								)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_2(r1,r2											)		}{_ci<<(r1); _ci<<(r2);																							GOTO_IZVRAT_2 (r1,r2							)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_3(r1,r2,r3										)		}{_ci<<(r1); _ci<<(r2); _ci<<(r3);																			GOTO_IZVRAT_3 (r1,r2,r3						)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_4(r1,r2,r3,r4									)		}{_ci<<(r1); _ci<<(r2); _ci<<(r3); _ci<<(r4);																GOTO_IZVRAT_4 (r1,r2,r3,r4					)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_5(r1,r2,r3,r4,r5								)		}{_ci<<(r1); _ci<<(r2); _ci<<(r3); _ci<<(r4); _ci<<(r5);												GOTO_IZVRAT_5 (r1,r2,r3,r4,r5				)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_6(r1,r2,r3,r4,r5,r6							)		}{_ci<<(r1); _ci<<(r2); _ci<<(r3); _ci<<(r4); _ci<<(r5); _ci<<(r6);								GOTO_IZVRAT_6 (r1,r2,r3,r4,r5,r6			)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_7(r1,r2,r3,r4,r5,r6,r7						)		}{_ci<<(r1); _ci<<(r2); _ci<<(r3); _ci<<(r4); _ci<<(r5); _ci<<(r6); _ci<<(r7);					GOTO_IZVRAT_7 (r1,r2,r3,r4,r5,r6,r7		)	theArgResCache.Insert (_ci);	_cache_count++;}
#define CACHE_STORE_RESULT_8(r1,r2,r3,r4,r5,r6,r7,r8					)		}{_ci<<(r1); _ci<<(r2); _ci<<(r3); _ci<<(r4); _ci<<(r5); _ci<<(r6); _ci<<(r7); _ci<<(r8);	GOTO_IZVRAT_8 (r1,r2,r3,r4,r5,r6,r7,r8	)	theArgResCache.Insert (_ci);	_cache_count++;}
